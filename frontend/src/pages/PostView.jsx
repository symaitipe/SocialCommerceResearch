...
import { getPostComments, getPost, getPostSummary, updateCommentStatus } from '../services/api';
import './PostView.css';

const INTENT_CONFIG = [
  { key: 'negative_feedback',     label: 'Negative Feedback',  emoji: '🚨' },
  { key: 'purchase_intent',       label: 'Purchase Intent',    emoji: '🛒' },
  { key: 'order_details',         label: 'Order Details',      emoji: '🔒' },
  { key: 'payment_method',        label: 'Payment Method',     emoji: '💳' },
  ...
  { key: 'order_confirmation',    label: 'Order Confirmation', emoji: '✅' },
  { key: 'positive_feedback',     label: 'Positive Feedback',  emoji: '⭐' },
  { key: 'noise',                 label: 'General',             emoji: '💬' },
  { key: 'general',               label: 'General',             emoji: '💬' },
];

const URGENCY = {
  negative_feedback:     'urgent',
...
const AVATAR_COLORS = ['#1877f2', '#e11d48', '#7c3aed', '#059669', '#d97706', '#0891b2'];
...
const REPLY_TEMPLATES = {
  purchase_intent: {
    english:  (date) => `To confirm your order, please inbox us 📩\nYour details are safe with us 🔒`,
    sinhala:  (date) => `ඇණවුම සඳහා කරුණාකර inbox කරන්න 📩\nඔබේ විස්තර අපි රැකගන්නෙමු 🔒`,
    singlish: (date) => `Order confirm karanna inbox karanna 📩\nPrivate ah details denna 🔒`,
  ...
  negative_feedback: {
    english:  () => `We're sorry to hear this. Please inbox us so we can resolve this 🙏`,
    ...
    <div className={`pv-comment ${comment.status}`}>
      <div className="pv-comment-top">
        <div className="pv-commenter">
          ...
          <div>
            <span className="pv-commenter-name">{comment.commenter_name || 'Customer'}</span>
            <span className="pv-comment-time">{timeAgo(comment.created_at)}</span>
          </div>
        </div>
      </div>
...
const PostView = () => {
  ...
  const [collapsed, setCollapsed] = useState(
    Object.fromEntries(INTENT_CONFIG.map(c => [c.key, true]))
  );
  ...
  const handleMarkAllRead = async () => {
    ...
    setComments(prev =>
      prev.map(c => c.status === 'unread'
        ? { ...c, status: 'read_not_replied' }
        : c
      )
    );
  ...
  const groupedByIntent = INTENT_CONFIG
    .map(config => ({
      ...config,
      items: tabComments
      ...
      <div className="pv-header">
        ...
        <div className="pv-header-info">
          ...
          <span className="pv-meta">
            {post.total_comments} comments · synced {timeAgo(post.last_fetched_at)}
            {post.last_sync_new_count > 0 && (
              <span className="pv-new-badge">+{post.last_sync_new_count} new</span>
            )}
          </span>
        </div>
        <a
          ...
          {INTENT_CONFIG
            ...
            .map(c => (
              <span key={c.key} className="pv-summary-pill">
                {c.emoji} {c.label} <strong>{summary.intent_counts[c.key]}</strong>
              </span>
            ))
          }
        </div>
      ...
      <div className="pv-feed">
        {groupedByIntent.length === 0 && (
          <div className="pv-empty">
            ...
            {activeTab === 'read_not_replied' && '📭 No comments waiting for reply.'}
        ...
        {groupedByIntent.map(group => (
          ...
          >
            ...
            {!collapsed[group.key] && group.items.map(comment => (
              <CommentRow
                key={comment.id}
                comment={comment}
                activeTab={activeTab}
