...
const INTENT_CONFIG = [
  ...
  { key: "payment_method", label: "Payment Method", emoji: "💳", priority: 3 },
  { key: "price_inquiry", label: "Price Inquiry", emoji: "💰", priority: 4 },
  {
  ...
  { key: "location_availability", label: "Location", emoji: "📍", priority: 7 },
  {
  ...
  { key: "order_details", label: "Order Details", emoji: "🔒", priority: 9 },
  {
  ...
  { key: "noise", label: "General", emoji: "💬", priority: 11 },
  { key: "general", label: "General", emoji: "💬", priority: 12 },
];
    ...
    <div className="pv-page">
      ...
      {summary && (
        <div className="pv-summary">
          {INTENT_CONFIG.filter((c) => summary.intent_counts?.[c.key] > 0).map(
            (c) => (
      ...
      <div className="pv-feed">
        ...
        {groupedByIntent.map((group) => (
          <div key={group.key} className="pv-group">
  ...
  return (
    <div className={`pv-comment ${comment.status}`}>
      <div className="pv-comment-top">
        ...
        <div className="pv-comment-badges">
          <span className="pv-lang-badge">{comment.language}</span>
          {comment.confidence === "high" && (
            <span className="pv-conf-badge high">high confidence</span>
          )}
          {comment.ai_assisted === 1 && (
            <span className="pv-conf-badge ai">AI</span>
          )}
        </div>
      </div>
      ...
      <div className="pv-comment-actions">
        ...
        {activeTab === "read_not_replied" && (
          <>
            ...
            <button
              className="pv-btn replied"
              onClick={() => onStatusChange(comment.id, "replied")}
            >
              ✅ Mark Replied
            </button>
          </>
    ...
    <div className="reply-box">
      {templates.length > 0 && (
        <div className="reply-templates">
          <span className="reply-template-label">💡 Suggested:</span>
          {templates.map((t, i) => (
            <button
              key={i}
